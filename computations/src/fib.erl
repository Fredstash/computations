...
-spec handle_call(Request::term(), From::pid(), State::term()) ->
                                  ...
                                  {stop, term(), term()}.
handle_call(calc_fib, From, State) ->
        {reply, {ok, calc_fib(State)}, State};
...
calc_fib(0) -> 0;
calc_fib(1) -> 1;
calc_fib(Num) -> calc_fib(Num-1) + calc_fib(Num-2).
...
handle_call_test_() -> 
    [
        ?_assertEqual({reply, {ok, 55}, 10}, fib:handle_call(calc_fib, from, 10))
    ].
